...
import { warn, deepExtend, simpleExtend, isFunction, isString, isPlainObject, toArray, isArray, isUndefined } from './utils'
...
// baseComponentId 保证 component 类型的唯一性，时间戳保证组件唯一性
// function uuid() {
//   return '$naive-component-' + baseComponentId++ + '-' + new Date().getTime()
// }
...
function createComponentCreator (context, componentDefine) {
  return function createComponent(props, children, key) {
    const newChild = new Component(deepExtend({}, componentDefine, {props,  key}))
    context._components[key] = newChild
    return newChild
  }
}

const prtt = Component.prototype
